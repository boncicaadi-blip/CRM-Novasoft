...
import { uploadContractDraftAction, toggleActivDraftAction, stergeDraftAction, genereazaContractTestAction, type DateFinanciareContract } from "@/lib/actions/contracte";
...
function TestGenerareModal({
  ...
  const tipServiciu = tipuriServiciu.find((t) => t.id === draft.tip_contract_id)?.valoare?.toUpperCase() ?? "";
  ...
  function handleGenerate() {
    ...
    startTransition(async () => {
      const result = await genereazaContractTestAction(partnerId, draft.id, dateFinanciare);
      setRezultat(result);
    });
  }
      ...
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-xl border border-border-subtle bg-surface-1 p-4">
        ...
        <div className="space-y-2.5">
          ...
          <p className="text-[11px] text-text-secondary">
            ...
            plus cele financiare de mai sus, daca sunt aplicabile. Tabelul detaliat de etape/functionalitati ramane
            necompletat, de adaugat manual. Placeholder-ele fara valoare raman marcate cu &quot;___&quot;.
          </p>
        </div>
