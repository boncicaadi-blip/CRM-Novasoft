  ...
  return (
    <div className="px-4 py-4 sm:px-6 sm:py-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-heading text-text-primary">Distributie clienti pe harta</h1>
          <p className="text-sm text-text-muted">Venituri, distribuite geografic pe judete, Romania</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="rounded-md border border-border-subtle bg-surface-2 px-3 py-1.5 text-sm text-text-primary outline-none focus:border-[#E8007A]"
          />
          <div className="flex gap-1 rounded-lg bg-surface-1 p-1 text-xs">
            <button
              onClick={() => setMetric("count")}
              className={`rounded-md px-3 py-1.5 transition ${metric === "count" ? "bg-surface-2 text-text-primary" : "text-text-muted"}`}
            >
              Nr. linii
            </button>
            <button
              onClick={() => setMetric("arr")}
              className={`rounded-md px-3 py-1.5 transition ${metric === "arr" ? "bg-surface-2 text-text-primary" : "text-text-muted"}`}
            >
              Venit realizat
            </button>
          </div>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-[1fr_280px]">
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-1 p-4">
          <p className="mb-3 text-sm font-medium text-text-primary">Top judete</p>
          <div className="space-y-2">
            ...
            {sortedJudete.length === 0 && <p className="text-xs text-text-muted">Niciun venit cu judet completat.</p>}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border-subtle bg-surface-1 p-4">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-sm font-medium text-text-primary">
            ...
            <span className="ml-2 text-xs text-text-muted">({clientiSortati.length})</span>
          </p>
          {selectedJudete.length > 0 && (
            <button
              onClick={() => setSelectedJudete([])}
              className="flex items-center gap-1 rounded-md p-1 text-xs text-text-muted transition hover:bg-surface-1 hover:text-text-primary"
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <tbody>
              {clientiSortati.map((c) => (
                <tr key={c.partnerId} className="border-b border-border-faint hover:bg-surface-1">
                  <td className="truncate px-3 py-2 text-text-secondary">{c.grup}</td>
                  <td className="truncate px-3 py-2 text-text-primary">{c.nume}</td>
                  <td className="px-3 py-2 text-text-secondary">{c.judet}</td>
                  <td className="px-3 py-2 text-right font-mono text-text-primary">{formatEur(c.realizat)}</td>
                  <td className="px-3 py-2 text-right font-mono text-text-secondary">{formatEur(c.estimat)}</td>
                </tr>
              ))}
