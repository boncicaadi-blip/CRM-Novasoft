...
/**
 * Doar afisare - targetul nu se mai seteaza manual, se calculeaza automat
 * ca suma valorilor propuse (facturi bifate "Propus spre incasare"). Ca sa
 * schimbi targetul, bifezi/debifezi facturi sau editezi valoarea propusa
 * din fisa unei facturi.
 */
export function CreanteGrtCard({
  monthKey,
  target,
  realizat,
}: {
    ...
    <div className="rounded-xl border border-border-subtle bg-surface-1 p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="flex items-center gap-1.5 text-sm font-medium text-text-primary">
          ...
          GRT — {labelLuna}
          <InfoTooltip title="GRT (Grad Realizare Target)" definition={CREANTE_KPI_DEFINITIONS.grtCard} />
        </p>
      ...
      {target === 0 ? (
        <p className="py-2 text-xs text-text-muted">
          Niciun target pentru luna curenta inca - bifeaza facturi ca &quot;Propus spre incasare&quot; in
          lista de Creante, ca sa vezi aici gradul de realizare.
        </p>
      ) : (
        <>
