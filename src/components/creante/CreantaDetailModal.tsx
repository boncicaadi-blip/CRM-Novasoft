    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between">
          <div>
            <p className="text-xs text-text-muted">Factura {creanta.nr_factura}</p>
            <h2 className="text-lg font-heading text-text-primary">{creanta.nume_firma}</h2>
            <p className="text-xs text-text-muted">{creanta.serviciu_facturat ?? "—"}</p>
          </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 rounded-lg border border-border-subtle bg-surface-1 p-3 text-sm">
          <div>
            ...
            <p className="font-mono text-text-primary">{formatRon(creanta.total_factura)}</p>
          </div>
          <div>
            <p className="text-[11px] text-text-muted">Sold</p>
            <p className="font-mono text-text-primary">{formatRon(creanta.sold)}</p>
          </div>
          <div>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-1.5">
              {incasari.map((i) => (
                ...
                >
                  <div>
                    <span className="font-mono text-text-primary">{formatRon(i.valoare)}</span>
        ...
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-[11px] text-text-muted">Tip vanzare</label>
            <select
              value={tipVanzare}
          ...
          {status === "restanta" && (
            <div className="rounded-md border border-amber-500/20 bg-amber-500/5 p-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="mb-1 block text-[10px] text-text-muted">Data promisa</label>
                  <input
                    type="date"
                    value={dataPromisa}
                    onChange={(e) => setDataPromisa(e.target.value)}
                    className="w-full rounded-md border border-border-subtle bg-surface-2 px-2 py-1.5 text-sm text-text-primary outline-none focus:border-[#E8007A]"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[10px] text-text-muted">Suma promisa</label>
                  <input
                    type="number"
                    step="0.01"
                    value={sumaPromisa}
