 ...
 */
export type DateFinanciareContract =
  | { tip: "implementare"; valoareTotala: number }
  | { tip: "saas"; nrUseri1: number; nrUseri2: number; valoarePerUser: number }
  | null;

export async function genereazaContractTestAction(
  partnerId: string,
  draftId: string,
  dateFinanciare: DateFinanciareContract = null,
  opportunityId: string | null = null
): Promise<{ success: boolean; message: string; contractId?: string; downloadUrl?: string; noteValidare?: string }> {
  const check = await requireAdminOrEditor();
  if (!check.ok) return { success: false, message: check.message };
  ...
  const { mergeContractTemplate, extrageTextDinDocx } = await import("@/lib/contracte/merge");
  const { mapeazaPartenerLaPlaceholdere, calculeazaTranseImplementare, calculeazaTotaluriSaas } = await import(
    "@/lib/contracte/mapare"
  );
  const { valideazaContractCuClaude } = await import("@/lib/contracte/validare");

  const templateBuffer = Buffer.from(await fileData.arrayBuffer());
  let placeholders = mapeazaPartenerLaPlaceholdere(partner);

  // Campurile financiare simple - calculate automat (matematica, nu Claude):
  // eșalonul de plata la implementare (4 transe de 25%), sau totalurile de
  // abonament la SaaS. Tabelul mare de etape de implementare ramane
  // intentionat needucat, completat manual.
  if (dateFinanciare?.tip === "implementare") {
    placeholders = { ...placeholders, ...calculeazaTranseImplementare(dateFinanciare.valoareTotala) };
  } else if (dateFinanciare?.tip === "saas") {
    placeholders = {
      ...placeholders,
      ...calculeazaTotaluriSaas({
        nrUseri1: dateFinanciare.nrUseri1,
        nrUseri2: dateFinanciare.nrUseri2,
        valoarePerUser: dateFinanciare.valoarePerUser,
      }),
    };
  }
  ...
  // Validare cu Claude - citeste documentul rezultat si semnaleaza ce a
  // ramas necompletat sau ce pare inconsecvent. Nu blocheaza generarea
  // daca esueaza (contractul tot se salveaza, doar fara observatii).
  let noteValidare: string | null = null;
  try {
    const textContract = extrageTextDinDocx(mergedBuffer);
    noteValidare = await valideazaContractCuClaude(textContract);
  } catch (err) {
    console.error("genereazaContractTestAction - validare error:", err);
  }
  ...
  if (opportunityId) revalidatePath(`/oportunitati/${opportunityId}`);
  ...
  return {
    success: true,
    message: "Contract generat.",
    contractId: inserted.id,
    downloadUrl: signedUrlData?.signedUrl,
    noteValidare: noteValidare ?? undefined,
  };
}
